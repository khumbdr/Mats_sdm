...
library(dplyr)
library(sf)
list_mat<-list.files("C:/Users/Khum/Documents/mbm_output/mat_reproject",pattern=".tif$", full.names=TRUE)

ty<- read_sf("C:/Users/Khum/OneDrive - UCB-O365/MBM_sdm_data/TaylorValley_shape/Polygons.shp")%>%
  st_transform(crs(raster(list_mat[3])))

library(raster)
dem_masked<-crop(raster(list_mat[1]),ty)
snowice_masked<-crop(raster(list_mat[2]),ty)
soil_masked<-crop(raster(list_mat[3]),ty)
vwc_masked<-crop(raster(list_mat[4]),ty)
vwcsetzerp<-crop(raster(list_mat[5]),ty)
water_masked<-crop(raster(list_mat[6]),ty)

elevation_re<-resample(dem_masked,vwc_masked,method="bilinear")
soil_re<-resample(soil_masked,vwc_masked, method="bilinear")
water_re<-resample(water_masked,vwc_masked, method="bilinear")
snowice_re<-resample(snowice_masked,vwc_masked, method="bilinear")
vwcsetzerp_re<-resample(vwcsetzerp,vwc_masked, method="bilinear")

slope_re<-terrain(elevation_re,opt=c("slope"), unit="radians")
aspect_re<-terrain(elevation_re,opt=c("aspect"),unit="radians")


mat_predictor<-stack(elevation_re,slope_re,aspect_re,soil_re,water_re,vwc_masked,snowice_re,vwcsetzerp_re)

alt<-raster("C:/Users/Khum/OneDrive - UCB-O365/MBM_sdm_data/ex_rs_projection/IND_msk_alt.gri")


mat_env<-projectRaster(mat_predictor,crs=crs(alt))

writeRaster(mat_env,"C:/Users/Khum/Documents/mbm_output/mat_stacked/mat_env",format="raster")

library(raster)
library(dplyr)
matpre<-stack("C:/Users/Khum/Documents/mbm_output/mat_stacked/mat_env.gri")

plot(matpre$dem_pr)
points(orgmat$ycentroid,orgmat$xcentroid)
```
...
# extracting the predictor variables values on gps points
pre_ext<-raster::extract(matpre,orgmat_01[,c("ycentroid","xcentroid")])

cor_env<-pre_ext%>%
  cor(use = "complete.obs")

write.csv(cor_env,"C:/Users/Khum/OneDrive - UCB-O365/Mats_sdm/data_output/cor_env.csv")
...
#########
#Getting the models out

## getting model values

write.csv(bm_PlotEvalMean(bm.out=myBiomodModelOut)[1],file="C:/Users/Khum/OneDrive - UCB-O365/Mats_sdm/data_output/mat_model.csv")


get_value<-get_variables_importance(myBiomodModelOut, as.data.frame = TRUE)

write.csv(get_value,"C:/Users/Khum/OneDrive - UCB-O365/Mats_sdm/data_output/variable_imp.csv")

var_imp<-read.csv("C:/Users/Khum/OneDrive - UCB-O365/Mats_sdm/data_output/variable_imp.csv")
