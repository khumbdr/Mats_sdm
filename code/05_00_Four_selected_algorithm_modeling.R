...
## running Maxent usung ENMeval package
# Here, we used above crated training and testing data frame for MAXENT analysis
...
# running model
e.maxent.jar <- ENMevaluate(occs, pre_env, bg,
                            tune.args = list(fc = c("L","LQ"), rm = 1:2),
                            partitions = "block", other.settings = os, partition.settings = ps,
                            algorithm = "maxent.jar", overlap = TRUE)

# getting probability values on area of interest
blk_maxnet_predict <- enm.maxent.jar@predict(mods.maxent.jar$fc.L_rm.2, pre_env, os)
